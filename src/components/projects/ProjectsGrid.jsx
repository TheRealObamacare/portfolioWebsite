...
const ProjectsGrid = ({ showFilter = true }) => {
  const {
    projects,
    searchProject,
    setSearchProject,
    ...
    <section className="py-5 sm:py-10 mt-5 sm:mt-10">
      ...
      <div className="mt-10 sm:mt-16">
        <div className="flex justify-between border-b border-primary-light dark:border-secondary-dark pb-3 gap-3">
          ...
          {/* Only render the filter when showFilter is true */}
          {showFilter && <ProjectsFilter />}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-6 sm:gap-10">
          {selectProject
            ? selectProjectsByCategory.map((project) => {
                console.log("Rendering project with ID:", project.id);
                return (
                  <ProjectSingle
                    id={project.id}
                    title={project.title}
